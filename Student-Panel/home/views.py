...
def signin(request):
    if request.method=='POST':
        ...
        # user = auth.authenticate(Email=email,pwd=pwd_user)
        if User.objects.filter(Email=email).exists():
...
def updatecourse(request):
    uid = request.POST['id']
    c = Courses.objects.filter(id=uid).get()
...
def dashboard(request):
    ...
    return render (request,'dashboard.html',{'nm':nm,"count_teacher":count_teacher,'data':data,'count_course':count_course,'count_student':count_student})
...
def addteacher(request):
        if request.method == "POST":
            ...
            Teacher_mobile_number= request.POST["tmobile"]
            Teacher_joining_date= request.POST["tjoindate"]
            Teacher_education= request.POST["teducation"]
            Teacher_employee_id= request.POST["tempid"]
            Teacher_work_exp= request.POST["tworkexpe"]
            teacher_pack=request.POST["tpack"]
